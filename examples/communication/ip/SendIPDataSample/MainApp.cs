...
namespace ConsoleApp.Communication.IP.SendIPDataSample
{
	/// <summary>
	/// XBee C# Library Send IP Data sample application.
	/// </summary>
	/// <remarks>This example sends IP data to another Cellular device specified by its IP
	...
	public class MainApp
	{
		...
		private const IPProtocol PROTOCOL = IPProtocol.TCP;
			...
			try
			{
				...
				else
				{
					Console.WriteLine("Sending data to {0}:{1} >> {2} | {3}... ",
						...
						DATA_TO_SEND);
					myDevice.SendIPData(IPAddress.Parse(DEST_IP_ADDRESS), DEST_PORT, PROTOCOL, dataToSend);
